...
locals {
  instances = {
    ...
    (var.master_node_0_hostname) = {
      ...
      public_interface_primary = true
      firewall_id              = linode_firewall.k8-master-node-firewall.id
      vpc_ip                   = "10.0.0.3"
    }
    (var.worker_node_0_hostname) = {
      ...
      public_interface_primary = true
      firewall_id              = linode_firewall.k8-worker-node-firewall.id
      vpc_ip                   = "10.0.0.4"
    }
...
apt-get -q update
# apt-get -q -y upgrade  # this seems to cause issues. 
